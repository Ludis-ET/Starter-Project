...
const CustomDropdown: React.FC<CustomDropdownProps> = ({
  ...
  const handleAssignReviewer = async (
    ...
    try {
      const response = await fetch(
        BASE_URL + `/manager/applications/${app.id}/assign`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const actions: Action[] = [
    ...
    } },
    {
      name: "View Details",
      onClick: () => {
        window.location.href = `/manager/${app.id}`;
      },
    },
    {
  ...
  return (
    ...
    <div className="relative inline-block text-left" ref={dropdownRef}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="inline-flex justify-between w-32 p-1 bg-white rounded-md text-blue-600 text-sm hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-200"
      >
        Actions ▼
      </button>
      ...
      {isOpen && (
        <div className="absolute right-0 w-32 mt-1 bg-white border border-gray-300 rounded-md shadow-lg z-100 transition-all duration-200 ease-in-out">
          {actions.map((action) => (
            ...
            >
              {action.name}
              {action.name === "Assign to Reviewer" && (
          ...
          <div className="max-h-40 overflow-y-auto">
            {filteredReviewers.map((reviewer) => (
              <div
                ...
                className="flex items-center px-2 py-1 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer"
              >
                <span className="text-xs w-4 h-4 bg-gray-400 rounded-full mr-2"></span>
                {reviewer.name}
              </div>
            ))}
          </div>
