...
const ImprovedReviewerDashboard = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap = {
      pending: { variant: "secondary" as const, label: "Pending Review", tooltip: "Application is waiting to be reviewed" },
      in_progress: { variant: "default" as const, label: "Under Review", tooltip: "Application is currently being reviewed" },
      completed: { variant: "destructive" as const, label: "Review Complete", tooltip: "Review process completed" },
      submitted: { variant: "secondary" as const, label: "Submitted", tooltip: "Application has been submitted" },
    };

    const statusInfo = statusMap[status as keyof typeof statusMap] || statusMap.pending;
    return (
      <Badge variant={statusInfo.variant} title={statusInfo.tooltip} className="cursor-help select-none">
        {statusInfo.label}
      </Badge>
    );
  };
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600 mt-2 text-lg">
              You have <span className="font-semibold">{totalCount}</span> applications waiting for your review.
            </p>
          </div>
          ...
          {["All", "pending", "in_progress", "completed"].map((status) => (
            <Button
              ...
              className={`
                ...
                ${filter === status ? "bg-indigo-600 hover:bg-indigo-700" : "hover:bg-blue-50"}
                ${filter === status ? "scale-105 shadow-lg text-white" : ""}
              `}
              aria-pressed={filter === status}
              aria-label={`Filter applications by ${status === "All" ? "all statuses" : status.replace("_", " ")}`}
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredApps.map((app) => (
          <Card
            ...
            role="button"
            onKeyDown={(e) => {
              if (e.key === "Enter" || e.key === " ") handleReview(app.application_id);
            }}
          ...
          >
            ...
            <CardContent className="space-y-5">
              ...
              <div className="flex items-center space-x-2 text-sm text-gray-600">
                ...
                <span className="bg-amber-200 rounded px-3 text-orange-900">Ready for Review</span>
              </div>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between bg-white rounded-lg border p-4 space-y-3 sm:space-y-0">
          ...
          <nav aria-label="Pagination navigation" className="flex items-center space-x-2">
            <Button
              variant="outline"
              size="sm"
      ...
      {filteredApps.length === 0 && !loading && (
        <div className="text-center py-16 space-y-4">
          ...
          <h3 className="text-2xl font-semibold text-gray-900">No applications found</h3>
